...
class DeltaBlobRecurrence(BlobRecurrence):
    ...
    def __post_init__(self):
        ...
        if schedulable_timerange.duration() > self.delta:
            raise ValueError("Blob schedulable timerange duration should not be larger than delta")
    ...
    def all_occurrences(self, timerange: TimeRange) -> List[Blob]:
        ...
        if start <= start_schedulable_timerange.start:
            ...
            intervals_passed = time_diff // self.delta
            curr_datetime = start_schedulable_timerange.start + intervals_passed * self.delta
        ...
        while curr_datetime <= end:
            ...
            blob_copy = blob_copy_with_delta_future(self.start_blob, delta_to_occurrence)
...
class DateBlobRecurrence(BlobRecurrence):
    ...
    def __post_init__(self):
        ...
        if start.weekday() != end.weekday():
            raise ValueError("Date blob recurrence should have a blob with timerange that starts and ends on the same day")
        
        if start.year != end.year:
            raise ValueError("Date blob recurrence should have a blob with timerange that starts and ends on the same year")
    ...
    def all_occurrences(self, timerange: TimeRange) -> List[Blob]:
        ...
        while True:
            ...
            if target_date > end or (not timerange.contains(blob_copy.get_schedulable_timerange())):
                break
