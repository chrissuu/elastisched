...
def test_different_timezone_le_comparison_raise_value_error():
    ...
    default_timerange2 = TimeRange(
        start=datetime(year=2001, month=1, day=1, hour=1, tzinfo=timezone(timedelta(hours=-5))),
        end=datetime(year=2001, month=1, day=1, hour=12, tzinfo=timezone(timedelta(hours=-5)))
    )

    schedulable_timerange2 = TimeRange(
        start=datetime(year=2001, month=1, day=1, hour=1, tzinfo=timezone(timedelta(hours=-5))),
        end=datetime(year=2001, month=1, day=2, hour=23, tzinfo=timezone(timedelta(hours=-5)))
    )
